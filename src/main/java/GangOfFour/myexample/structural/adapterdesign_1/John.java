package GangOfFour.myexample.structural.adapterdesign_1;
...
public class John implements ITarget {
	...
	@Override
	public String translateAndTellToOtherPerson(String words,
			String convertToWhichLanguage) {
		if (convertToWhichLanguage.equalsIgnoreCase("English")) {

			String englishWords = convertToEnglish(words);
			System.out.println("\nJohn Converted \"" + words + " \" to \""
					+ englishWords + " and send the question to David");
			String englishWordsReply = david.answerFortheQuestion(englishWords);
			System.out.println("John Got reply from David in English like : "
					+ "\"" + englishWordsReply + "\"");
			String frenchConverted = convertToFrench(englishWordsReply);
			System.out.println("John Converted " + "\"" + englishWordsReply
					+ "\"" + " to " + "\"" + frenchConverted + "\""
...

		{
			...
			System.out.println("\nJohn Converted \"" + words + " \" to \""
					+ frenchWords + " and send the question to Rohit");
			String frenchWordsReply = rohit.answerFortheQuestion(frenchWords);
			System.out.println("John Got reply from Rohit in French like : "
					+ "\"" + frenchWordsReply + "\"");
			String englishConverted = convertToEnglish(frenchWordsReply);
			System.out.println("John Converted " + "\"" + frenchWordsReply
					+ "\"" + " to " + "\"" + englishConverted + "\""
